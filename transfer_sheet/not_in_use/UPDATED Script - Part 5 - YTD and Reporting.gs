/**
 * ============================================================================
 * PART 5: YTD SUMMARY AND REPORTING FUNCTIONS (UPDATED)
 * ============================================================================
 * Version: 2.1 | Updated: 2025-11-24
 * Changes: Updated Finance and Site reports, added Program Budget Summary
 */
...
function calculateInOutTotals(site, program, year) {
  ...
  var data = logSheet.getRange(6, 1, lastRow - 5, 21).getValues();
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    if (logSite === site && logProgram === program && logYear === year) {
      ...
      // IN/OUT is USD-based
      totals.budgetedUSD += (row[9] || 0) * multiplier;      // Column J: USD
      totals.budgetedLocal += (row[11] || 0) * multiplier;   // Column L: Local equivalent
      totals.actualUSD += (row[13] || 0) * multiplier;       // Column N: Actual USD
      totals.actualLocal += (row[14] || 0) * multiplier;     // Column O: Actual Local
    }
  }
...
function generateItemizedTransferRequest() {
  ...
  var siteResponse = ui.prompt(
    'Generate Transfer Request (Line Items)',
  ...
  var report = 'ITEMIZED TRANSFER REQUEST (LINE ITEMS) - ' + site + ' - ' + month + '\n';
  ...
  ui.showModalDialog(htmlOutput, 'Itemized Transfer Request (Line Items) - ' + site);
}
...
function generateProgramBudgetSummary() {
  ...
  var report = 'OVERALL MINISTRY BUDGET (BY PROGRAM) - ' + site + '\n';
  report += 'Currency: ' + currency + ' (Rate: ' + exchangeRate + ')\n';
  report += '='.repeat(70) + '\n\n';
  ...
  for (var i = 0; i < PROGRAMS.length; i++) {
    ...
    if (programTotal.local > 0 || programTotal.usd > 0) {
      report += program.padEnd(40) + 
                formatCurrency(programTotal.local, currency).padStart(15) + 
                '  $' + programTotal.usd.toFixed(2).padStart(10) + '\n';
      
      grandTotalLocal += programTotal.local;
      grandTotalUSD += programTotal.usd;
    }
  }
  
  report += '='.repeat(70) + '\n';
  report += 'TOTAL MINISTRY BUDGET:'.padEnd(40) + 
            formatCurrency(grandTotalLocal, currency).padStart(15) + 
            '  $' + grandTotalUSD.toFixed(2).padStart(10) + '\n';
...
function calculateProgramTotal(site, program) {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var currentYear = new Date().getFullYear();
  
  // Sum from all three budget types
  var localIncome = calculateLocalIncomeTotals(site, program, currentYear);
  var usTransfer = calculateUSTransferTotals(site, program, currentYear);
  var inOut = calculateInOutTotals(site, program, currentYear);
  ...
  return {
    local: localIncome.budgetedLocal + usTransfer.budgetedLocal + inOut.budgetedLocal,
    usd: localIncome.budgetedUSD + usTransfer.budgetedUSD + inOut.budgetedUSD,
    currency: currency
  };
}
